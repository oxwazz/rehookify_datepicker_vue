 ...
 * 2028 2029 2030
 */
export function getStartDecadePosition(year: number): number {
 ...
 */
export function getFluidYearPosition(year: number, numberOfYears: number): number {
  return year - (numberOfYears / 2 - (numberOfYears % 2 === 0 ? 1 : 0))
}
 ...
 * 2021 2022 2023
 */
export function getStartExactPosition(year: number, numberOfYears: number): number {
  return year - numberOfYears + 1
}

export const isExactMode = (mode: DPYearsMode): boolean => mode === 'exact'

export function getCurrentYearPosition(year: number, { mode, numberOfYears }: DPYearsConfig): number {
  if (isExactMode(mode))
